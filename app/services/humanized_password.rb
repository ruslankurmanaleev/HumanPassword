class HumanizedPassword
  ...
  def generate
    ...
    return short_password if @length < 9

    # We need more digits (2-4) for passwords from 9 to 15 length
    return medium_password if @length < 16
  ...
  def short_password
    # Fill with syllables
    fill_with_syllables(pairs: @number_of_pairs)

    # Fill with 1 vowel if there any symbols_left
    fill_with_digits(symbols: 1) if available_space_for_digits?
